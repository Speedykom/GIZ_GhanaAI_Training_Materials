...
## 3. Why We Need Agentic Frameworks

### The Problem with Raw LLM Integration

Building agentic systems from scratch requires handling:

1. **Prompt Engineering**: Crafting the right instructions for the LLM
2. **Context Management**: Keeping track of conversation history
3. **Tool Integration**: Connecting APIs, databases, and services
4. **Error Handling**: Managing failures gracefully
5. **State Management**: Persisting data between interactions
6. **Testing**: Verifying complex, non-deterministic behavior

This is like building a web application without Django or Flask—you *can* do it, but you're reinventing the wheel.

### What Frameworks Provide

Good agentic frameworks handle the boilerplate so you can focus on your business logic:

| Concern | Framework Handles | You Focus On |
|:--------|:------------------|:-------------|
| LLM Communication | API calls, retries, rate limiting | What to ask |
| Tool Integration | API wrappers, authentication | Which tools to use |
| Workflow Logic | Sequencing, branching, looping | Your process flow |
| State Management | Context, memory, persistence | What to remember |
| Error Handling | Retries, fallbacks, logging | Recovery strategies |
| Testing | Test utilities, mock LLMs | Test scenarios |

### Introducing CodeTrain

**CodeTrain** is a minimalist workflow framework inspired by [PocketFlow](https://github.com/The-Pocket/PocketFlow)—a 100-line LLM framework that strips away complexity to reveal essential patterns.

CodeTrain adapts these patterns for African startups and developers using intuitive, logistics-based terminology:

- **Job** = A unit of work (like a delivery stop)
- **Hustle** = Orchestrates multiple Jobs into a workflow  
- **Manifest** = Carries data between stages
...
5. **Iterate**: Ask OpenCode to add features:
   ```
